...
RenderPass::~RenderPass() noexcept {
    if (m_renderPass != VK_NULL_HANDLE && m_deviceHandle != VK_NULL_HANDLE) {
        vkDestroyRenderPass(m_deviceHandle, m_renderPass, nullptr);
    }
}

bool RenderPass::init(VkDevice device, VkFormat swapchainImageFormat) noexcept {
    ...
    if (vkCreateRenderPass(m_deviceHandle, &renderPassInfo, nullptr, &m_renderPass) != VK_SUCCESS) {
        std::cerr << "[RenderPass] Failed to create render pass" << std::endl;
        return false;
    }
